...
import { Users, Calendar, Trophy, LogOut, UserCircle, Newspaper, Eye, Trash2, Edit, X, Home, BookOpen, DollarSign, ShoppingBag } from 'lucide-react';
import { supabase } from '@/lib/supabase'
import { useRouter } from 'next/navigation'
import RichTextEditor from '@/components/RichTextEditor';
import { uploadImage, validateImageFile, deleteImage } from '@/lib/upload-image';

type TabType = 'students' | 'fighters' | 'events' | 'fights' | 'announcements' | 'filiales' | 'classes' | 'plans' | 'products';
...
export default function AdminPage() {
  ...
  const [stats, setStats] = useState({ students: 0, fighters: 0, events: 0, fights: 0, announcements: 0, filiales: 0, classes: 0 })
  ...
  const [fighterForm, setFighterForm] = useState({
    id: null as number | null,
    name: '', nickname: '', division: '', gym: '', photo_url: '', is_active: true,
    record_profesional: '', record_amateur: '', link_tapology: '', categoria: 'amateur',
    disciplina: '', entrenador: '', altura_cm: '', peso_kg: '', alcance_cm: '', guardia: '', campeonatos: ''
  })
  ...
  const [filialForm, setFilialForm] = useState({
    ...
    mapLink: '', horarios: {}, servicios: [] as string[], precios: {}, descripcion: '', disponible: true, image_url: ''
  })
  ...
  const [fightForm, setFightForm] = useState({ event_id: '', red_fighter_id: '', opponent_name: '', result: 'red', method: '', round: 1 })
  ...
  const [productForm, setProductForm] = useState({
    id: null as number | null,
    name: '', category: '', price: '', description: '', image_url: '', stock: true
  })
  ...
  useEffect(() => {
    if (user) {
      loadStats(); loadStudents(); loadFighters(); loadAllFighters()
      loadEvents(); loadAnnouncements(); loadConfigData(); loadProducts()
    }
  }, [user])
  ...
  async function checkAuth() {
    ...
    if (!admin && !receptionist) { await supabase.auth.signOut(); router.push('/login'); return }
    setIsAdmin(admin)
    setIsReceptionist(receptionist)
    setActiveTab(admin ? 'students' : 'announcements')
  ...
  async function loadStats() {
    try {
      ...
      setStats({
        students: studentsRes.count || 0, fighters: fightersRes.count || 0,
        events: eventsRes.count || 0, fights: fightsRes.count || 0,
        announcements: announcementsRes.count || 0, filiales: filialesData.length, classes: classesData.length
      })
  ...
  async function saveConfigData(key: string, data: any) {
    try {
      const { error } = await supabase.from('config').upsert({ key, data, updated_at: new Date() }, { onConflict: 'key' })
      if (error) throw error
      setMessage({ type: 'success', text: '✅ Cambios guardados exitosamente' })
      loadConfigData(); loadStats()
    } catch (error: any) {
  ...
  async function handleCreateOrUpdateStudent(e: React.FormEvent) {
    ...
    try {
      ...
      if (studentImageFile) {
        ...
        if (studentForm.photo_url) await deleteImage(studentForm.photo_url, 'students-photos')
        photoUrl = uploadedUrl
      }
      const studentData = {
        name: studentForm.name, email: studentForm.email || null, phone: studentForm.phone || null,
        birth_date: studentForm.birth_date || null, discipline: studentForm.discipline || null,
        belt_level: studentForm.belt_level || null, enrollment_date: studentForm.enrollment_date || null,
        gym: studentForm.gym || null, status: studentForm.status, photo_url: photoUrl || null,
      ...
      setStudentForm({ id: null, name: '', email: '', phone: '', birth_date: '', discipline: '', belt_level: '', enrollment_date: '', gym: '', status: 'active', photo_url: '', notes: '', weight_kg: '', height_cm: '' })
      setStudentImageFile(null); setStudentImagePreview('')
  ...
  function handleEditStudent(student: any) {
    ...
    setStudentImagePreview(student.photo_url || ''); setStudentImageFile(null)
  ...
  async function handleDeleteStudent(id: number, photoUrl: string | null) {
    ...
    try {
      if (photoUrl?.includes('students-photos')) await deleteImage(photoUrl, 'students-photos')
  ...
  async function loadFighters() {
    const { data } = await supabase.from('fighters').select('*').eq('activo', true).order('nombre')
    if (data) setFighters(data.map((f: any) => ({ id: f.id, name: f.nombre, nickname: f.apodo, division: f.division, gym: f.gimnasio, photo_url: f.foto_url, is_active: f.activo })))
  }

  async function loadAllFighters() {
    const { data } = await supabase.from('fighters').select('*').order('nombre')
    if (data) setAllFighters(data.map((f: any) => ({
      id: f.id, name: f.nombre, nickname: f.apodo, division: f.division, gym: f.gimnasio,
      photo_url: f.foto_url, is_active: f.activo, record_profesional: f.record_profesional,
      record_amateur: f.record_amateur, link_tapology: f.link_tapology, categoria: f.categoria,
      disciplina: f.disciplina, entrenador: f.entrenador, altura_cm: f.altura_cm,
      peso_kg: f.peso_kg, alcance_cm: f.alcance_cm, guardia: f.guardia, campeonatos: f.campeonatos
    })))
  }
  ...
  async function handleCreateOrUpdateFighter(e: React.FormEvent) {
    ...
    try {
      ...
      if (fighterImageFile) {
        ...
        if (!uploadedUrl) throw new Error('Error al subir la imagen')
        if (fighterForm.photo_url) await deleteImage(fighterForm.photo_url, 'fighters-photos')
        photoUrl = uploadedUrl
      }
      const fighterData = {
        nombre: fighterForm.name, apodo: fighterForm.nickname || null,
        division: fighterForm.division || null, gimnasio: fighterForm.gym || null,
        foto_url: photoUrl || null, activo: fighterForm.is_active,
        record_profesional: fighterForm.record_profesional || null,
        record_amateur: fighterForm.record_amateur || null,
        link_tapology: fighterForm.link_tapology || null,
        categoria: fighterForm.categoria || null, disciplina: fighterForm.disciplina || null,
        entrenador: fighterForm.entrenador || null,
        altura_cm: fighterForm.altura_cm ? parseFloat(fighterForm.altura_cm) : null,
        peso_kg: fighterForm.peso_kg ? parseFloat(fighterForm.peso_kg) : null,
        alcance_cm: fighterForm.alcance_cm ? parseFloat(fighterForm.alcance_cm) : null,
        guardia: fighterForm.guardia || null, campeonatos: fighterForm.campeonatos || null,
      }
      if (fighterForm.id) {
      ...
      setFighterForm({ id: null, name: '', nickname: '', division: '', gym: '', photo_url: '', is_active: true, record_profesional: '', record_amateur: '', link_tapology: '', categoria: 'amateur', disciplina: '', entrenador: '', altura_cm: '', peso_kg: '', alcance_cm: '', guardia: '', campeonatos: '' })
      setFighterImageFile(null); setFighterImagePreview('')
      loadStats(); loadFighters(); loadAllFighters()
    } catch (error: any) {
      setMessage({ type: 'error', text: `❌ Error: ${error.message}` })
    }
  }
  ...
  function handleEditFighter(fighter: any) {
    setFighterForm({
      ...
      record_profesional: fighter.record_profesional || '', record_amateur: fighter.record_amateur || '',
      link_tapology: fighter.link_tapology || '', categoria: fighter.categoria || 'amateur',
      disciplina: fighter.disciplina || '', entrenador: fighter.entrenador || '',
      altura_cm: fighter.altura_cm ? String(fighter.altura_cm) : '',
      peso_kg: fighter.peso_kg ? String(fighter.peso_kg) : '',
      alcance_cm: fighter.alcance_cm ? String(fighter.alcance_cm) : '',
      guardia: fighter.guardia || '', campeonatos: fighter.campeonatos || '',
    })
    setFighterImagePreview(fighter.photo_url || ''); setFighterImageFile(null)
    window.scrollTo({ top: 0, behavior: 'smooth' })
  }
  ...
  async function handleDeleteFighter(id: number, photoUrl: string | null) {
    ...
    try {
      if (photoUrl?.includes('fighters-photos')) await deleteImage(photoUrl, 'fighters-photos')
      const { error } = await supabase.from('fighters').delete().eq('id', id)
      if (error) throw error
      await revalidatePage('/fighters'); await revalidatePage('/fighters/[id]')
  ...
  async function handleToggleFighterActive(fighter: any) {
    try {
      const { error } = await supabase.from('fighters').update({ activo: !fighter.is_active }).eq('id', fighter.id)
      if (error) throw error
      setMessage({ type: 'success', text: `✅ Peleador ${!fighter.is_active ? 'activado' : 'desactivado'} exitosamente` })
      loadFighters(); loadAllFighters(); loadStats()
    } catch (error: any) {
      setMessage({ type: 'error', text: `❌ Error: ${error.message}` })
    }
  }
  ...
  async function handleCreateOrUpdateFilial(e: React.FormEvent) {
    ...
    try {
      ...
      const newFilial = { ...filialForm, image_url: imageUrl, slug: filialForm.name.toLowerCase().replace(/\s+/g, '-') }
      let updatedFiliales = editingFilialId
        ? filialesData.map(f => f.id === editingFilialId ? { ...newFilial, id: editingFilialId } : f)
        : [...filialesData, newFilial]
      await saveConfigData('filiales', updatedFiliales)
      setFilialForm({ id: Date.now(), name: '', slug: '', status: 'Activa', address: '', phone: '', email: '', whatsapp: '', mapLink: '', horarios: {}, servicios: [], precios: {}, descripcion: '', disponible: true, image_url: '' })
      setFilialImageFile(null); setFilialImagePreview(''); setEditingFilialId(null)
    } catch (error: any) {
      setMessage({ type: 'error', text: `❌ Error: ${error.message}` })
    }
  }
  ...
  function handleEditFilial(filial: any) {
    setFilialForm(filial); setFilialImagePreview(filial.image_url || '')
    setFilialImageFile(null); setEditingFilialId(filial.id)
    window.scrollTo({ top: 0, behavior: 'smooth' })
  }
  ...
  async function handleDeleteFilial(id: number) {
    ...
    try {
      ...
      await saveConfigData('filiales', updatedFiliales); await revalidatePage('/')
    } catch (error: any) {
      setMessage({ type: 'error', text: `❌ Error: ${error.message}` })
    }
  }
  ...
  async function handleCreateOrUpdateClass(e: React.FormEvent) {
    ...
    try {
      ...
      const newClass = { ...classForm, imageUrl, slug: classForm.slug || classForm.name.toLowerCase().replace(/\s+/g, '-') }
      let updatedClasses = editingClassSlug
        ? classesData.map(c => c.slug === editingClassSlug ? newClass : c)
        : [...classesData, newClass]
      await saveConfigData('classes', updatedClasses)
      setClassForm({ slug: '', name: '', shortDescription: '', fullDescription: '', ageRange: '', schedule: { days: '', times: [] }, pricing: { monthly: '', inscription: '' }, color: 'bg-blue-600', imageUrl: '' })
      setClassImageFile(null); setClassImagePreview(''); setEditingClassSlug(null)
    } catch (error: any) {
      setMessage({ type: 'error', text: `❌ Error: ${error.message}` })
    }
  }
  ...
  function handleEditClass(clase: any) {
    setClassForm(clase); setClassImagePreview(clase.imageUrl || '')
    setClassImageFile(null); setEditingClassSlug(clase.slug)
    window.scrollTo({ top: 0, behavior: 'smooth' })
  }
  ...
  async function handleDeleteClass(slug: string) {
    ...
    try {
      ...
      await saveConfigData('classes', updatedClasses); await revalidatePage('/clases')
    } catch (error: any) {
      setMessage({ type: 'error', text: `❌ Error: ${error.message}` })
    }
  }
  ...
  async function handleCreateOrUpdatePlan(e: React.FormEvent) {
    e.preventDefault(); setMessage(null)
    try {
      let updatedPlans = editingPlanId
        ? plansData.map(p => p.id === editingPlanId ? planForm : p)
        : [...plansData, planForm]
      await saveConfigData('plans', updatedPlans)
      setPlanForm({ id: Date.now(), name: '', subtitle: '', price: '', period: 'mensual', features: [], highlighted: false })
  ...
  function handleEditPlan(plan: any) {
    setPlanForm(plan); setEditingPlanId(plan.id)
    window.scrollTo({ top: 0, behavior: 'smooth' })
  }
  ...
  async function handleDeletePlan(id: number) {
    ...
    try {
      ...
      await saveConfigData('plans', updatedPlans); await revalidatePage('/')
    } catch (error: any) {
      setMessage({ type: 'error', text: `❌ Error: ${error.message}` })
    }
  }
  ...
  async function handleCreateOrUpdateEvent(e: React.FormEvent) {
    e.preventDefault(); setMessage(null)
    try {
      const eventData = { name: eventForm.name, org: eventForm.org || null, event_date: eventForm.event_date || null, location: eventForm.location || null }
      if (eventForm.id) {
  ...
  function handleEditEvent(event: any) {
    setEventForm({ id: event.id, name: event.name, org: event.org || '', event_date: event.event_date || '', location: event.location || '' })
    window.scrollTo({ top: 0, behavior: 'smooth' })
  }
  ...
  async function handleDeleteEvent(id: number) {
    ...
    try {
      ...
      if (error) throw error
      await revalidatePage('/events'); await revalidatePage('/events/[id]')
  ...
  async function handleCreateFight(e: React.FormEvent) {
    ...
    try {
      const { error } = await supabase.from('fights').insert({
        ...
        opponent_name: fightForm.opponent_name || null,
        result: fightForm.result, method: fightForm.method || null, round: fightForm.round || null
      })
      if (error) throw error
      await supabase.rpc('refresh_fighter_records')
      setMessage({ type: 'success', text: '✅ Combate registrado y récords actualizados' })
      setFightForm({ event_id: '', red_fighter_id: '', opponent_name: '', result: 'red', method: '', round: 1 })
      loadStats()
    } catch (error: any) {
      setMessage({ type: 'error', text: `❌ Error: ${error.message}` })
    }
  ...
  async function handleCreateOrUpdateAnnouncement(e: React.FormEvent) {
    ...
    try {
      ...
      const announcementData = {
        title: announcementForm.title, content: announcementForm.content, excerpt,
        featured_image_url: imageUrl || null, published: announcementForm.published, author_email: user?.email
      }
      ...
      setAnnouncementForm({ id: null, title: '', content: '', excerpt: '', featured_image_url: '', published: false })
  ...
  function handleEditAnnouncement(announcement: any) {
    ...
    setImagePreview(announcement.featured_image_url || ''); setImageFile(null)
    window.scrollTo({ top: 0, behavior: 'smooth' })
  }
  ...
  async function handleDeleteAnnouncement(id: number) {
    ...
    try {
      ...
      if (error) throw error
      await revalidatePage('/blog'); await revalidatePage('/announcements')
  ...
  async function handleCreateOrUpdateProduct(e: React.FormEvent) {
    ...
    try {
      ...
      if (productImageFile) {
        ...
        if (!uploadedUrl) throw new Error('Error al subir la imagen')
        if (productForm.image_url) await deleteImage(productForm.image_url, 'products-images')
        imageUrl = uploadedUrl
      }
      const productData = {
        name: productForm.name, category: productForm.category || null,
        price: parseFloat(productForm.price), description: productForm.description || null,
        image_url: imageUrl || null, stock: productForm.stock
      }
      if (productForm.id) {
      ...
      setProductForm({ id: null, name: '', category: '', price: '', description: '', image_url: '', stock: true })
  ...
  function handleEditProduct(product: any) {
    ...
    setProductImagePreview(product.image_url || ''); setProductImageFile(null)
    window.scrollTo({ top: 0, behavior: 'smooth' })
  }

  async function handleDeleteProduct(id: number, imageUrl: string | null) {
    if (!confirm('¿Estás seguro de eliminar este producto?')) return
    try {
      if (imageUrl?.includes('products-images')) await deleteImage(imageUrl, 'products-images')
  ...
  async function handleToggleStock(product: any) {
    try {
  ...
  const TabButton = ({ id, label, icon: Icon }: { id: TabType; label: string; icon: any }) => (
    <button onClick={() => setActiveTab(id)} className={`py-4 px-2 border-b-2 font-medium text-sm transition whitespace-nowrap ${activeTab === id ? 'border-red-600 text-red-600' : 'border-transparent text-gray-500 hover:text-gray-700'}`}>
      <div className="flex items-center space-x-2"><Icon className="w-4 h-4" /><span>{label}</span></div>
    </button>
  )
    ...
    <div className="min-h-screen bg-gray-50">
        ...
        <div className="container mx-auto px-4">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center space-x-4">
              <div className="flex items-center space-x-2 text-sm"><UserCircle className="w-5 h-5" /><span>{user?.email}</span></div>
              <button onClick={handleLogout} className="flex items-center space-x-2 bg-red-600 hover:bg-red-700 px-4 py-2 rounded-lg transition">
      ...
      <div className="container mx-auto px-4 -mt-8">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            { label: 'Alumnos', value: stats.students, color: 'text-blue-600', Icon: Users },
            { label: 'Peleadores', value: stats.fighters, color: 'text-red-600', Icon: Trophy },
            { label: 'Eventos', value: stats.events, color: 'text-purple-600', Icon: Calendar },
            { label: 'Anuncios', value: stats.announcements, color: 'text-orange-600', Icon: Newspaper },
          ].map(({ label, value, color, Icon }) => (
            <div key={label} className="bg-white rounded-lg shadow-md p-6">
              <div className="flex items-center justify-between">
                <div><p className="text-gray-600 text-xs md:text-sm">{label}</p><p className="text-2xl md:text-3xl font-bold text-gray-900">{value}</p></div>
                <Icon className={`w-8 md:w-12 h-8 md:h-12 ${color} opacity-80`} />
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="bg-white rounded-lg shadow-md">
          <div className="border-b border-gray-200 overflow-x-auto">
            <div className="flex space-x-2 px-6 min-w-max">
              {isAdmin && <TabButton id="students" label="Alumnos" icon={Users} />}
              {isAdmin && <TabButton id="fighters" label="Peleadores" icon={Trophy} />}
              {isAdmin && <TabButton id="filiales" label="Filiales" icon={Home} />}
              {isAdmin && <TabButton id="classes" label="Clases" icon={BookOpen} />}
              {isAdmin && <TabButton id="plans" label="Planes" icon={DollarSign} />}
              {isAdmin && <TabButton id="events" label="Eventos" icon={Calendar} />}
              {isAdmin && <TabButton id="fights" label="Combates" icon={Trophy} />}
              <TabButton id="announcements" label="Anuncios" icon={Newspaper} />
              <TabButton id="products" label="Tienda" icon={ShoppingBag} />
              {isReceptionist && <TabButton id="students" label="Alumnos" icon={Users} />}
            </div>
          </div>
          ...
          <div className="p-6">
            ...
            {activeTab === 'students' && <AdminStudentsTab studentForm={studentForm} setStudentForm={setStudentForm} studentImageFile={studentImageFile} studentImagePreview={studentImagePreview} handleStudentImageSelect={handleStudentImageSelect} handleCreateOrUpdateStudent={handleCreateOrUpdateStudent} uploadingStudentImage={uploadingStudentImage} students={students} handleEditStudent={handleEditStudent} handleDeleteStudent={handleDeleteStudent} />}
            {activeTab === 'fighters' && <AdminFightersTab fighterForm={fighterForm} setFighterForm={setFighterForm} fighterImageFile={fighterImageFile} fighterImagePreview={fighterImagePreview} handleFighterImageSelect={handleFighterImageSelect} handleCreateOrUpdateFighter={handleCreateOrUpdateFighter} uploadingFighterImage={uploadingFighterImage} allFighters={allFighters} handleEditFighter={handleEditFighter} handleDeleteFighter={handleDeleteFighter} handleToggleFighterActive={handleToggleFighterActive} />}
            {activeTab === 'filiales' && <AdminFilialesTab filialForm={filialForm} setFilialForm={setFilialForm} filialImageFile={filialImageFile} filialImagePreview={filialImagePreview} handleFilialImageSelect={handleFilialImageSelect} handleCreateOrUpdateFilial={handleCreateOrUpdateFilial} uploadingFilialImage={uploadingFilialImage} filialesData={filialesData} handleEditFilial={handleEditFilial} handleDeleteFilial={handleDeleteFilial} editingFilialId={editingFilialId} />}
            {activeTab === 'classes' && <AdminClassesTab classForm={classForm} setClassForm={setClassForm} classImageFile={classImageFile} classImagePreview={classImagePreview} handleClassImageSelect={handleClassImageSelect} handleCreateOrUpdateClass={handleCreateOrUpdateClass} uploadingClassImage={uploadingClassImage} classesData={classesData} handleEditClass={handleEditClass} handleDeleteClass={handleDeleteClass} editingClassSlug={editingClassSlug} />}
            {activeTab === 'plans' && <AdminPlansTab planForm={planForm} setPlanForm={setPlanForm} handleCreateOrUpdatePlan={handleCreateOrUpdatePlan} plansData={plansData} handleEditPlan={handleEditPlan} handleDeletePlan={handleDeletePlan} editingPlanId={editingPlanId} />}
            {activeTab === 'events' && <AdminEventsTab eventForm={eventForm} setEventForm={setEventForm} handleCreateOrUpdateEvent={handleCreateOrUpdateEvent} events={events} handleEditEvent={handleEditEvent} handleDeleteEvent={handleDeleteEvent} />}
            {activeTab === 'fights' && <AdminFightsTab fightForm={fightForm} setFightForm={setFightForm} handleCreateFight={handleCreateFight} events={events} fighters={fighters} />}
            {activeTab === 'announcements' && <AdminAnnouncementsTab announcementForm={announcementForm} setAnnouncementForm={setAnnouncementForm} imageFile={imageFile} imagePreview={imagePreview} handleImageSelect={handleImageSelect} handleCreateOrUpdateAnnouncement={handleCreateOrUpdateAnnouncement} uploadingImage={uploadingImage} announcements={announcements} handleEditAnnouncement={handleEditAnnouncement} handleDeleteAnnouncement={handleDeleteAnnouncement} handleTogglePublish={handleTogglePublish} setPreviewAnnouncement={setPreviewAnnouncement} />}
            {activeTab === 'products' && <AdminProductsTab productForm={productForm} setProductForm={setProductForm} productImageFile={productImageFile} productImagePreview={productImagePreview} handleProductImageSelect={handleProductImageSelect} handleCreateOrUpdateProduct={handleCreateOrUpdateProduct} uploadingProductImage={uploadingProductImage} products={products} handleEditProduct={handleEditProduct} handleDeleteProduct={handleDeleteProduct} handleToggleStock={handleToggleStock} />}
          </div>
        </div>
      </div>
          ...
          <div className="bg-white rounded-lg max-w-3xl w-full max-h-[90vh] overflow-y-auto">
            <div className="p-6">
              <div className="flex items-center justify-between mb-4">
                ...
                <button onClick={() => setPreviewAnnouncement(null)} className="text-gray-400 hover:text-gray-600">✕</button>
              </div>
              {previewAnnouncement.featured_image_url && <img src={previewAnnouncement.featured_image_url} alt={previewAnnouncement.title} className="w-full h-64 object-cover rounded-lg mb-6" />}
              <h1 className="text-3xl font-bold text-gray-900 mb-4">{previewAnnouncement.title}</h1>
              <div className="flex items-center gap-4 text-sm text-gray-500 mb-6">
                <span>{new Date(previewAnnouncement.created_at).toLocaleDateString('es-MX', { year: 'numeric', month: 'long', day: 'numeric' })}</span>
                <span>•</span><span>{previewAnnouncement.author_email}</span>
              </div>
              <div className="prose prose-sm max-w-none" dangerouslySetInnerHTML={{ __html: previewAnnouncement.content }} />
            </div>
          </div>
        </div>
      )}
...
function AdminStudentsTab({ studentForm, setStudentForm, studentImageFile, studentImagePreview, handleStudentImageSelect, handleCreateOrUpdateStudent, uploadingStudentImage, students, handleEditStudent, handleDeleteStudent }: any) {
  return (
    <div>
      <h2 className="text-xl font-bold text-gray-900 mb-6">{studentForm.id ? 'Editar Alumno' : 'Crear Nuevo Alumno'}</h2>
      <form onSubmit={handleCreateOrUpdateStudent} className="space-y-6 mb-12 pb-12 border-b">
        <div>
          ...
          <input type="file" accept="image/*" onChange={handleStudentImageSelect} className="block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-semibold file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100" />
          <p className="text-xs text-gray-500 mt-1">JPG, PNG o WebP. Máximo 5MB</p>
          {studentImagePreview && (
            <div className="relative inline-block mt-3">
              <img src={studentImagePreview} alt="Preview" className="w-32 h-32 object-cover rounded-lg border-2 border-gray-200" />
              <button type="button" onClick={() => setStudentForm({...studentForm, photo_url: ''})} className="absolute -top-2 -right-2 bg-red-600 text-white p-1 rounded-full hover:bg-red-700"><X className="w-4 h-4" /></button>
            </div>
          )}
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <input type="date" value={studentForm.birth_date} onChange={(e) => setStudentForm({...studentForm, birth_date: e.target.value})} className="px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent" />
          <select value={studentForm.discipline} onChange={(e) => setStudentForm({...studentForm, discipline: e.target.value})} className="px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
            ...
            <option value="MMA">MMA</option><option value="Muay Thai">Muay Thai</option>
            <option value="BJJ">BJJ</option><option value="Boxeo">Boxeo</option><option value="CrossFit">CrossFit</option>
          </select>
          ...
          <input type="date" value={studentForm.enrollment_date} onChange={(e) => setStudentForm({...studentForm, enrollment_date: e.target.value})} className="px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent" />
        ...
        <button type="submit" disabled={uploadingStudentImage} className="bg-blue-600 hover:bg-blue-700 text-white px-6 py-2 rounded-lg transition disabled:opacity-50">
        ...
        {students.map((student: any) => (
          <div key={student.id} className="bg-white border border-gray-200 rounded-lg p-4 hover:shadow-md transition">
            <div className="flex items-start space-x-4 mb-4">
              {student.photo_url ? <img src={student.photo_url} alt={student.name} className="w-16 h-16 rounded-lg object-cover" /> : <div className="w-16 h-16 rounded-lg bg-gray-200 flex items-center justify-center"><Users className="w-8 h-8 text-gray-400" /></div>}
            ...
            <div className="flex items-center gap-2">
              <button onClick={() => handleEditStudent(student)} className="flex-1 flex items-center justify-center gap-1 px-3 py-2 bg-blue-50 hover:bg-blue-100 text-blue-700 rounded transition text-sm"><Edit className="w-4 h-4" />Editar</button>
              <button onClick={() => handleDeleteStudent(student.id, student.photo_url)} className="px-3 py-2 bg-red-50 hover:bg-red-100 text-red-700 rounded transition"><Trash2 className="w-4 h-4" /></button>
            </div>
          </div>
        ))}
      </div>
...
function AdminFightersTab({ fighterForm, setFighterForm, fighterImageFile, fighterImagePreview, handleFighterImageSelect, handleCreateOrUpdateFighter, uploadingFighterImage, allFighters, handleEditFighter, handleDeleteFighter, handleToggleFighterActive }: any) {
  return (
    <div>
      <h2 className="text-xl font-bold text-gray-900 mb-6">{fighterForm.id ? 'Editar Peleador' : 'Crear Nuevo Peleador'}</h2>
      <form onSubmit={handleCreateOrUpdateFighter} className="space-y-6 mb-12 pb-12 border-b">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <input type="text" value={fighterForm.division} onChange={(e) => setFighterForm({...fighterForm, division: e.target.value})} placeholder="División / Categoría de peso" className="px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-red-500 focus:border-transparent" />
          <input type="text" value={fighterForm.gym} onChange={(e) => setFighterForm({...fighterForm, gym: e.target.value})} placeholder="Gimnasio / Filial" className="px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-red-500 focus:border-transparent" />
          <select value={fighterForm.disciplina} onChange={(e) => setFighterForm({...fighterForm, disciplina: e.target.value})} className="px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-red-500 focus:border-transparent">
            <option value="">Disciplina</option>
            <option value="MMA">MMA</option><option value="Muay Thai">Muay Thai</option>
            <option value="Boxeo">Boxeo</option><option value="BJJ">BJJ</option>
            <option value="Wrestling">Wrestling</option><option value="Kickboxing">Kickboxing</option>
          </select>
          <select value={fighterForm.categoria} onChange={(e) => setFighterForm({...fighterForm, categoria: e.target.value})} className="px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-red-500 focus:border-transparent">
            <option value="amateur">Amateur</option>
            <option value="semi-profesional">Semi Profesional</option>
            <option value="profesional">Profesional</option>
          </select>
          <input type="text" value={fighterForm.record_profesional} onChange={(e) => setFighterForm({...fighterForm, record_profesional: e.target.value})} placeholder="Récord Profesional (ej: 5-2-0)" className="px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-red-500 focus:border-transparent" />
          <input type="text" value={fighterForm.record_amateur} onChange={(e) => setFighterForm({...fighterForm, record_amateur: e.target.value})} placeholder="Récord Amateur (ej: 10-1-0)" className="px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-red-500 focus:border-transparent" />
          <input type="text" value={fighterForm.entrenador} onChange={(e) => setFighterForm({...fighterForm, entrenador: e.target.value})} placeholder="Entrenador" className="px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-red-500 focus:border-transparent" />
          <input type="url" value={fighterForm.link_tapology} onChange={(e) => setFighterForm({...fighterForm, link_tapology: e.target.value})} placeholder="Link Tapology (https://...)" className="px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-red-500 focus:border-transparent" />
          <input type="number" step="0.1" value={fighterForm.altura_cm} onChange={(e) => setFighterForm({...fighterForm, altura_cm: e.target.value})} placeholder="Altura (cm)" className="px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-red-500 focus:border-transparent" />
          <input type="number" step="0.1" value={fighterForm.peso_kg} onChange={(e) => setFighterForm({...fighterForm, peso_kg: e.target.value})} placeholder="Peso (kg)" className="px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-red-500 focus:border-transparent" />
          <input type="number" step="0.1" value={fighterForm.alcance_cm} onChange={(e) => setFighterForm({...fighterForm, alcance_cm: e.target.value})} placeholder="Alcance (cm)" className="px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-red-500 focus:border-transparent" />
          <select value={fighterForm.guardia} onChange={(e) => setFighterForm({...fighterForm, guardia: e.target.value})} className="px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-red-500 focus:border-transparent">
            <option value="">Guardia</option>
            <option value="Orthodox">Orthodox</option><option value="Southpaw">Southpaw</option><option value="Switch">Switch</option>
          </select>
        </div>
        <textarea value={fighterForm.campeonatos} onChange={(e) => setFighterForm({...fighterForm, campeonatos: e.target.value})} placeholder="Campeonatos y títulos obtenidos" rows={3} className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-red-500 focus:border-transparent" />
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {allFighters.map((fighter: any) => (
          <div key={fighter.id} className={`bg-white border rounded-lg p-4 hover:shadow-md transition ${!fighter.is_active ? 'border-gray-300 opacity-60' : 'border-gray-200'}`}>
            <div className="flex items-start space-x-4 mb-4">
              {fighter.photo_url ? <img src={fighter.photo_url} alt={fighter.name} className="w-16 h-16 rounded-lg object-cover" /> : <div className="w-16 h-16 rounded-lg bg-gray-200 flex items-center justify-center"><Users className="w-8 h-8 text-gray-400" /></div>}
              <div className="flex-1">
                ...
                {fighter.categoria && <span className="text-xs bg-gray-100 text-gray-600 px-2 py-0.5 rounded">{fighter.categoria}</span>}
                {fighter.division && <p className="text-xs text-gray-500 mt-1">{fighter.division}</p>}
                {fighter.record_profesional && <p className="text-xs text-gray-500">Pro: {fighter.record_profesional}</p>}
              </div>
            </div>
            <div className="flex items-center gap-2">
              <button onClick={() => handleEditFighter(fighter)} className="flex-1 flex items-center justify-center gap-1 px-3 py-2 bg-blue-50 hover:bg-blue-100 text-blue-700 rounded transition text-sm"><Edit className="w-4 h-4" />Editar</button>
              ...
              <button onClick={() => handleDeleteFighter(fighter.id, fighter.photo_url)} className="px-3 py-2 bg-red-50 hover:bg-red-100 text-red-700 rounded transition"><Trash2 className="w-4 h-4" /></button>
            </div>
          </div>
        ))}
      </div>
    ...
    <div>
      ...
      <form onSubmit={handleCreateOrUpdateFilial} className="space-y-6 mb-12 pb-12 border-b">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <input type="url" value={filialForm.mapLink} onChange={(e) => setFilialForm({...filialForm, mapLink: e.target.value})} placeholder="Link de Google Maps" className="px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent" />
          <select value={filialForm.status} onChange={(e) => setFilialForm({...filialForm, status: e.target.value})} className="px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent">
        ...
        {filialesData.map((filial: any) => (
          <div key={filial.id} className="bg-white border border-gray-200 rounded-lg p-4 hover:shadow-md transition">
            ...
            <div className="flex items-center gap-2">
              ...
              <button onClick={() => handleDeleteFilial(filial.id)} className="px-3 py-2 bg-red-50 hover:bg-red-100 text-red-700 rounded transition"><Trash2 className="w-4 h-4" /></button>
            </div>
          </div>
        ))}
      </div>
...
function AdminClassesTab({ classForm, setClassForm, classImageFile, classImagePreview, handleClassImageSelect, handleCreateOrUpdateClass, uploadingClassImage, classesData, handleEditClass, handleDeleteClass, editingClassSlug }: any) {
  return (
    <div>
      <h2 className="text-xl font-bold text-gray-900 mb-6">{editingClassSlug ? 'Editar Clase' : 'Crear Nueva Clase'}</h2>
      <form onSubmit={handleCreateOrUpdateClass} className="space-y-6 mb-12 pb-12 border-b">
        <div>
          ...
          {classImagePreview && <div className="mt-3"><img src={classImagePreview} alt="Preview" className="w-32 h-32 object-cover rounded-lg border-2 border-gray-200" /></div>}
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <select value={classForm.color} onChange={(e) => setClassForm({...classForm, color: e.target.value})} className="px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent">
            <option value="bg-blue-600">Azul</option><option value="bg-red-600">Rojo</option>
            <option value="bg-green-600">Verde</option><option value="bg-yellow-600">Amarillo</option><option value="bg-purple-600">Púrpura</option>
          </select>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {classesData.map((clase: any) => (
          <div key={clase.slug} className="bg-white border border-gray-200 rounded-lg p-4 hover:shadow-md transition">
            <div className="flex items-start space-x-4 mb-4">
              {clase.imageUrl ? <img src={clase.imageUrl} alt={clase.name} className="w-16 h-16 rounded-lg object-cover" /> : <div className="w-16 h-16 rounded-lg bg-gray-200 flex items-center justify-center"><BookOpen className="w-8 h-8 text-gray-400" /></div>}
              <div className="flex-1"><h4 className="font-bold text-gray-900">{clase.name}</h4><p className="text-xs text-gray-500">{clase.ageRange}</p><p className="text-sm text-gray-600 font-semibold">{clase.pricing.monthly}</p></div>
            </div>
            <div className="flex items-center gap-2">
              <button onClick={() => handleEditClass(clase)} className="flex-1 flex items-center justify-center gap-1 px-3 py-2 bg-blue-50 hover:bg-blue-100 text-blue-700 rounded transition text-sm"><Edit className="w-4 h-4" />Editar</button>
              <button onClick={() => handleDeleteClass(clase.slug)} className="px-3 py-2 bg-red-50 hover:bg-red-100 text-red-700 rounded transition"><Trash2 className="w-4 h-4" /></button>
            </div>
          </div>
        ))}
      </div>
...
function AdminPlansTab({ planForm, setPlanForm, handleCreateOrUpdatePlan, plansData, handleEditPlan, handleDeletePlan, editingPlanId }: any) {
  return (
    <div>
      <h2 className="text-xl font-bold text-gray-900 mb-6">{editingPlanId ? 'Editar Plan' : 'Crear Nuevo Plan'}</h2>
      <form onSubmit={handleCreateOrUpdatePlan} className="space-y-6 mb-12 pb-12 border-b">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <select value={planForm.period} onChange={(e) => setPlanForm({...planForm, period: e.target.value})} className="px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-yellow-500 focus:border-transparent">
            <option value="mensual">Mensual</option><option value="día">Día</option><option value="anual">Anual</option>
          </select>
        </div>
        <label className="flex items-center space-x-3 bg-gray-50 p-4 rounded-lg">
        ...
        <button type="submit" className="bg-yellow-600 hover:bg-yellow-700 text-white px-6 py-2 rounded-lg transition">{editingPlanId ? 'Actualizar' : 'Crear'}</button>
      </form>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {plansData.map((plan: any) => (
          <div key={plan.id} className={`border rounded-lg p-4 hover:shadow-md transition ${plan.highlighted ? 'bg-yellow-50 border-yellow-200' : 'bg-white border-gray-200'}`}>
            ...
            <div className="flex items-center gap-2">
              <button onClick={() => handleEditPlan(plan)} className="flex-1 flex items-center justify-center gap-1 px-3 py-2 bg-blue-50 hover:bg-blue-100 text-blue-700 rounded transition text-sm"><Edit className="w-4 h-4" />Editar</button>
              <button onClick={() => handleDeletePlan(plan.id)} className="px-3 py-2 bg-red-50 hover:bg-red-100 text-red-700 rounded transition"><Trash2 className="w-4 h-4" /></button>
            </div>
          </div>
        ))}
      </div>
...
function AdminEventsTab({ eventForm, setEventForm, handleCreateOrUpdateEvent, events, handleEditEvent, handleDeleteEvent }: any) {
  return (
    <div>
      <h2 className="text-xl font-bold text-gray-900 mb-6">{eventForm.id ? 'Editar Evento' : 'Crear Nuevo Evento'}</h2>
      <form onSubmit={handleCreateOrUpdateEvent} className="space-y-4 mb-12 pb-12 border-b">
        ...
        <button type="submit" className="bg-purple-600 hover:bg-purple-700 text-white px-6 py-2 rounded-lg transition">{eventForm.id ? 'Actualizar' : 'Crear'}</button>
      </form>
        ...
        {events.map((event: any) => (
          <div key={event.id} className="bg-white border border-gray-200 rounded-lg p-4 hover:shadow-md transition">
            <div className="flex items-start justify-between">
              ...
              <div className="flex items-center gap-2">
                <button onClick={() => handleEditEvent(event)} className="p-2 hover:bg-blue-100 rounded transition"><Edit className="w-4 h-4 text-blue-600" /></button>
                <button onClick={() => handleDeleteEvent(event.id)} className="p-2 hover:bg-red-100 rounded transition"><Trash2 className="w-4 h-4 text-red-600" /></button>
              </div>
            </div>
          </div>
        ))}
  ...
  return (
    <div>
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-4">
        <p className="text-blue-800 text-sm">Selecciona nuestro peleador y escribe el nombre del oponente externo. El récord se actualiza automáticamente.</p>
      </div>
      <form onSubmit={handleCreateFight} className="space-y-4">
        <select required value={fightForm.event_id} onChange={(e) => setFightForm({...fightForm, event_id: e.target.value})} className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-red-500 focus:border-transparent">
          ...
          {events.map((event: any) => <option key={event.id} value={event.id}>{event.name} - {event.event_date}</option>)}
        </select>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Nuestro Peleador</label>
            <select required value={fightForm.red_fighter_id} onChange={(e) => setFightForm({...fightForm, red_fighter_id: e.target.value})} className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-red-500 focus:border-transparent">
              <option value="">Seleccionar peleador...</option>
              {fighters.map((fighter: any) => <option key={fighter.id} value={fighter.id}>{fighter.name}</option>)}
            </select>
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Oponente (externo)</label>
            <input type="text" required value={fightForm.opponent_name} onChange={(e) => setFightForm({...fightForm, opponent_name: e.target.value})} placeholder="Nombre del oponente externo" className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-red-500 focus:border-transparent" />
          </div>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <select required value={fightForm.result} onChange={(e) => setFightForm({...fightForm, result: e.target.value})} className="px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-red-500 focus:border-transparent">
            <option value="red">Victoria Rojo</option><option value="blue">Victoria Azul</option>
            <option value="draw">Empate</option><option value="nc">Sin Resultado</option>
          </select>
        ...
        <button type="submit" className="bg-red-600 hover:bg-red-700 text-white px-6 py-2 rounded-lg transition">Registrar Combate</button>
      </form>
    </div>
  )
}
...
function AdminAnnouncementsTab({ announcementForm, setAnnouncementForm, imageFile, imagePreview, handleImageSelect, handleCreateOrUpdateAnnouncement, uploadingImage, announcements, handleEditAnnouncement, handleDeleteAnnouncement, handleTogglePublish, setPreviewAnnouncement }: any) {
  return (
    <div>
      <h2 className="text-xl font-bold text-gray-900 mb-6">{announcementForm.id ? 'Editar Anuncio' : 'Crear Nuevo Anuncio'}</h2>
      <form onSubmit={handleCreateOrUpdateAnnouncement} className="space-y-6 mb-12 pb-12 border-b">
        ...
        <div>
          ...
          {imagePreview && <div className="mt-3"><img src={imagePreview} alt="Preview" className="w-full max-w-md h-48 object-cover rounded-lg border-2 border-gray-200" /></div>}
        </div>
        <div>
        ...
        {announcements.map((announcement: any) => (
          <div key={announcement.id} className="bg-white border border-gray-200 rounded-lg p-4 hover:shadow-md transition">
            <div className="flex items-start justify-between">
              ...
              <div className="flex items-center gap-2">
                <button onClick={() => setPreviewAnnouncement(announcement)} className="p-2 hover:bg-gray-100 rounded transition"><Eye className="w-4 h-4 text-gray-600" /></button>
                <button onClick={() => handleEditAnnouncement(announcement)} className="p-2 hover:bg-blue-100 rounded transition"><Edit className="w-4 h-4 text-blue-600" /></button>
                ...
                <button onClick={() => handleDeleteAnnouncement(announcement.id)} className="p-2 hover:bg-red-100 rounded transition"><Trash2 className="w-4 h-4 text-red-600" /></button>
              </div>
            </div>
          </div>
        ))}
...
function AdminProductsTab({ productForm, setProductForm, productImageFile, productImagePreview, handleProductImageSelect, handleCreateOrUpdateProduct, uploadingProductImage, products, handleEditProduct, handleDeleteProduct, handleToggleStock }: any) {
  return (
    <div>
      <h2 className="text-xl font-bold text-gray-900 mb-6">{productForm.id ? 'Editar Producto' : 'Agregar Nuevo Producto'}</h2>
      <form onSubmit={handleCreateOrUpdateProduct} className="space-y-6 mb-12 pb-12 border-b">
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-2">Foto del Producto</label>
          <input type="file" accept="image/*" onChange={handleProductImageSelect} className="block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-semibold file:bg-pink-50 file:text-pink-700 hover:file:bg-pink-100" />
          <p className="text-xs text-gray-500 mt-1">JPG, PNG o WebP. Máximo 5MB</p>
          {productImagePreview && (
            <div className="relative inline-block mt-3">
              <img src={productImagePreview} alt="Preview" className="w-32 h-32 object-cover rounded-lg border-2 border-gray-200" />
              <button type="button" onClick={() => setProductForm({...productForm, image_url: ''})} className="absolute -top-2 -right-2 bg-red-600 text-white p-1 rounded-full hover:bg-red-700"><X className="w-4 h-4" /></button>
            </div>
          )}
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <input type="text" required value={productForm.name} onChange={(e) => setProductForm({...productForm, name: e.target.value})} placeholder="Nombre del Producto *" className="px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-pink-500 focus:border-transparent" />
          <input type="text" value={productForm.category} onChange={(e) => setProductForm({...productForm, category: e.target.value})} placeholder="Categoría (Ropa, Equipo, Protección...)" className="px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-pink-500 focus:border-transparent" />
          <input type="number" required step="0.01" value={productForm.price} onChange={(e) => setProductForm({...productForm, price: e.target.value})} placeholder="Precio (MXN) *" className="px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-pink-500 focus:border-transparent" />
        </div>
        <textarea value={productForm.description} onChange={(e) => setProductForm({...productForm, description: e.target.value})} placeholder="Descripción del producto" rows={3} className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-pink-500 focus:border-transparent" />
        <label className="flex items-center space-x-3 bg-gray-50 p-4 rounded-lg">
          <input type="checkbox" checked={productForm.stock} onChange={(e) => setProductForm({...productForm, stock: e.target.checked})} className="w-5 h-5 text-pink-600 rounded focus:ring-pink-500" />
          <span className="text-sm font-medium text-gray-700">Disponible en stock</span>
        </label>
        <button type="submit" disabled={uploadingProductImage} className="bg-pink-600 hover:bg-pink-700 text-white px-6 py-2 rounded-lg transition disabled:opacity-50">
          {uploadingProductImage ? 'Subiendo...' : (productForm.id ? 'Actualizar' : 'Agregar Producto')}
        </button>
      </form>
      <h3 className="text-lg font-bold text-gray-900 mb-4">Productos ({products.length})</h3>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {products.map((product: any) => (
          <div key={product.id} className={`bg-white border rounded-lg p-4 hover:shadow-md transition ${!product.stock ? 'border-gray-300 opacity-70' : 'border-gray-200'}`}>
            <div className="flex items-start space-x-4 mb-4">
              {product.image_url ? <img src={product.image_url} alt={product.name} className="w-16 h-16 rounded-lg object-cover" /> : <div className="w-16 h-16 rounded-lg bg-gray-200 flex items-center justify-center"><ShoppingBag className="w-8 h-8 text-gray-400" /></div>}
              <div className="flex-1">
                <h4 className="font-bold text-gray-900 truncate">{product.name}</h4>
                {product.category && <p className="text-xs text-gray-500">{product.category}</p>}
                <p className="text-lg font-bold text-red-600">${product.price}</p>
                <span className={`text-xs px-2 py-0.5 rounded font-semibold ${product.stock ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
                  {product.stock ? 'En stock' : 'Agotado'}
                </span>
              </div>
            </div>
            <div className="flex items-center gap-2">
              <button onClick={() => handleEditProduct(product)} className="flex-1 flex items-center justify-center gap-1 px-3 py-2 bg-blue-50 hover:bg-blue-100 text-blue-700 rounded transition text-sm"><Edit className="w-4 h-4" />Editar</button>
              <button onClick={() => handleToggleStock(product)} className={`flex-1 px-3 py-2 rounded text-sm font-semibold transition ${product.stock ? 'bg-gray-200 hover:bg-gray-300 text-gray-700' : 'bg-green-600 hover:bg-green-700 text-white'}`}>
                {product.stock ? 'Agotar' : 'Activar'}
              </button>
              <button onClick={() => handleDeleteProduct(product.id, product.image_url)} className="px-3 py-2 bg-red-50 hover:bg-red-100 text-red-700 rounded transition"><Trash2 className="w-4 h-4" /></button>
            </div>
          </div>
        ))}
      </div>
